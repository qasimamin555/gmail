...
import { Unread } from "../store/actions/index";
...
class UnreadMessages extends Component {
  ...
  async componentDidMount() {
    ...
    axios
      .get(
        `https://gmail.googleapis.com/gmail/v1/users/qasimamin7877@gmail.com/messages/?maxResults=25&labelIds=UNREAD`,
        {
          headers: {
    ...
    for (let i = 0; i < this.state.array.length; i++) {
      axios
        ...
        .then(async (result) => {
          ...
          this.props.Unread(this.state.data);
        })
        .catch((err) => console.log(err));
    }
  };
  ...
  NextPage = async () => {
    ...
    await axios
      .get(
        `https://gmail.googleapis.com/gmail/v1/users/qasimamin7877@gmail.com/messages?maxResults=25&pageToken=${this.state.pageToken}&labelIds=UNREAD`,
        {
          headers: {
            Authorization: "Bearer " + access_token,
          },
  ...
  render() {
    return [
      <Display data={this.props.unread} history={this.props.history} />,
...
export default connect(mapStateToProps, { Unread })(UnreadMessages);
